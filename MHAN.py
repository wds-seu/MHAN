...
from FusionalAttention import FusionalAttention
import torch
...
class HGT(nn.Module):
    ...
    def forward(self, G):
        ...
        for i in range(self.n_layers):
            h = self.gcs[i](G, h)
        for ntype in G.ntypes:
            self.out_dict.update({ntype: self.linear(h[ntype])})
...
class MHAN(nn.Module):
    def __init__(self, G, in_size, hidden_size, out_size, n_layers, n_heads, n_fuse_heads, use_norm=True):
        ...
        for etype in G.etypes:
            ...
            G.edges[etype].data["id"] = (
                    torch.ones(G.number_of_edges(etype), dtype=torch.long).to(G.device)
                    * edge_dict[etype]
            )
        ...
        self.fusionalAttention = FusionalAttention(n_fuse_heads, out_size, out_size, 0.5)
    ...
    def forward(self, g, g2, neg_g, etype):
        ...
        h['NCT'] = self.fusionalAttention(z)[:, 0, :]
